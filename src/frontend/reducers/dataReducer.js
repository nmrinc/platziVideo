  ...
  switch (action.type) {
    ...
    case actionTypes.GET_VIDEO_SOURCE:
      return {
        ...
        data: {
          ...state.data,
          playing: state.data.trends.find(item => item.id === Number(action.payload))
            || state.data.originals.find(item => item.id === Number(action.payload))
            || []
        },
      }
      break;
    case actionTypes.SEARCH_VIDEO:
      if (action.payload) {
        return {
          ...
          data: {
            ...state.data,
            findings: [...state.data.trends, ...state.data.originals].filter(item => item.title.toLowerCase().includes(action.payload.toLowerCase()))
          },
        }
      } else {
        return {
